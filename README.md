...
| `on_error_cmd`     |           no                  | A command that will be called if the backup *fails* in any of the steps. Useful for heartbeat monitoring systems. Use the full command you need to run. Example: `curl https://monitoring.example.com/api/push/E9Wzm4lJ2O?status=failed&msg=fail` |
